...
def getDataLists():
    langs = {"Abaza", "Abkhaz", "Adyghe", "Afrikaans", "Akan", "Albanian", "American Sign Language", "Amharic",
			 "Arabic", "Aragonese", "Armenian", "Old Armenian", "Assamese", "Asturian", "Avar", "Aymara", "Azerbaijani",
			 "Bashkir", "Basque", "Belarusian", "Bengali", "Breton", "Bulgarian", "Burmese", "Buryat", "Catalan",
			 "Chakma", "Chechen", "Cherokee", "Chinese:", "Cantonese", "Dungan", "Mandarin", "Chukchi", "Chuvash",
			 "Cornish", "Crimean Tatar", "Czech", "Danish", "Dargwa", "Dhivehi", "Dutch", "Eastern Mari", "English",
			 "Erzya", "Esperanto", "Estonian", "Evenki", "Faroese", "Finnish", "Franco-Provençal", "French",
			 "Middle French", "Fula", "Gagauz", "Galician", "Georgian", "German", "Greek", "Ancient", "Greenlandic",
			 "Guerrero Amuzgo", "Gujarati", "Haitian Creole", "Hausa", "Hawaiian", "Hebrew", "Hiligaynon", "Hindi",
			 "Hopi", "Hungarian", "Icelandic", "Ido", "Igbo", "Indonesian", "Ingush", "Interlingua", "Interlingue",
			 "Inuktitut", "Irish", "Italian", "Japanese", "Javanese", "Kabardian", "Kalmyk", "Kannada", "Kapampangan",
			 "Karachay-Balkar", "Karakalpak", "Karelian", "Kashubian", "Kazakh", "Khakas", "Khmer", "Kikuyu",
			 "Komi-Permyak", "Korean", "Kumyk", "Kurdish:", "Kurmanji", "Sorani", "Kven", "Kyrgyz", "Lak", "Lao",
			 "Latgalian", "Latin", "Latvian", "Lezgi", "Ligurian", "Limburgish", "Lithuanian", "Low German", "Luhya",
			 "Lule Sami", "Luxembourgish", "Macedonian", "Malagasy", "Malay", "Malayalam", "Maltese", "Manchu", "Manx",
			 "Maori", "Marathi", "Mauritian Creole", "Meru", "Middle Persian", "Moksha", "Mongolian:", "Cyrillic",
			 "Mongolian", "Nahuatl", "Navajo", "Neapolitan", "Nenets:", "Tundra Nenets", "Nepali", "Nogai", "Norman",
			 "Northern Sami", "Norwegian:", "Bokmål", "Nynorsk", "Novial", "Occitan", "Ojibwe", "Oriya", "Ossetian:",
			 "Digor", "Iron", "Papiamentu", "Pashto", "Persian", "Plautdietsch", "Polish", "Portuguese", "Punjabi",
			 "Quechua", "Romanian", "Romansch", "Russian", "Rusyn", "Rwanda-Rundi", "Samogitian", "Sanskrit",
			 "Sardinian", "Scots", "Scottish Gaelic", "Serbo-Croatian:", "Cyrillic", "Roman", "Shona", "Shor",
			 "Sicilian", "Silesian", "Sindhi", "Sinhalese", "Skolt Sami", "Slovak", "Slovene", "Somali", "Sorbian:",
			 "Lower Sorbian", "Upper Sorbian", "Sotho", "Southern Altai", "Southern Ohlone", "Southern Sami", "Spanish",
			 "Swahili", "Swedish", "Tabasaran", "Tagalog", "Tajik", "Tamil", "Tatar", "Telugu", "Thai", "Tibetan",
			 "Tigrinya", "Tok Pisin", "Turkish", "Turkmen", "Tuvan", "Udmurt", "Ukrainian", "Urdu", "Uyghur", "Uzbek",
			 "Cyrillic", "Venda", "Veps", "Vietnamese", "Volapük", "Võro", "Wallisian", "Walloon", "Welsh",
			 "West Frisian", "Western Cham", "Wolof", "Xhosa", "Yakut", "Yiddish", "Yoruba", "Zazaki", "Zhuang", "Zulu"}
                ...
                for subsectionInd, subsection in enumerate(list(subsectionSplit[:-2])):
                    if "==" in subsection and "==\n" in subsectionSplit[subsectionInd + 2]:
                        ...
                        # capture translations
                            ...
                            subsectionContentList = [s for s in re.split(
                                r"\{\{trans-top\|?|\{\{checktrans-top\|?|\{\{trans-bottom\}\}|[\n]{2,6}",
                                subsectionContent) if s not in [""]]
                            ...
                            if subcontent != "":
                                ...
                                if re.fullmatch(r"\{\{.+\}\}", subcontent) is None:
                                    ...
                                    for indexSynset, (definitionSynset, synonymSynset) in enumerate(synsetData):
                                        dataDict[sectionLang][
                                            "{0}_{1}_{2}".format(wordEntry, synsetPos, indexSynset)] = {
                                            "language": sectionLang,
                                            "word": wordEntry,
                                            "pos data": wikiPos,
                                            "definition": definitionSynset,
                                            "definition token": word_tokenize(definitionSynset,
                                                                              language=sectionLang.lower()),
                                ...
                                # get metadata
                                    ...
                                    wikiPos = subcontent.replace("{{en-", "").replace("{{fr-", "").replace("{{",
                                                                                                           "").replace(
                                        "}}", "").split("|")
    # dump
    for lang in dataDict:
        lang_out_path_label = "{0}_{1}.label".format(outputPath, lang)
        lang_out_path_data = "{0}_{1}.json".format(outputPath, lang)
        # erase previous data dict
        with open(lang_out_path_label, 'w', encoding='utf8') as label_file:
            label_file.write('')
        with open(lang_out_path_data, 'w', encoding='utf8') as data_file:
            data_file.write('')
        # append each data line
        with open(lang_out_path_label, 'a', encoding='utf8') as label_file:
            with open(lang_out_path_data, 'w', encoding='utf8') as data_file:
                for uniq_id, data in dataDict[lang].items():
                    label_file.write("{0}\n".format(uniq_id))
